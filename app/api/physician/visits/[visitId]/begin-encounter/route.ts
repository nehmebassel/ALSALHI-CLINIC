...
export async function POST(
  ...
  try {
    ...
    return noStoreJson(
      await new PhysicianVisitLifecycleService(prisma).beginEncounter(
        actor,
        visitId,
      ),
    );
    ...
    console.error("Physician encounter start failed.", error);
    return noStoreJson(
      { error: { code: "INTERNAL_ERROR", message: "Physician encounter start failed." } },
      { status: 500 },
    );
  }
}
