...
export async function POST(
  ...
  try {
    ...
    const command = parsePatientContextReconciliationCommand(
      await parseBoundedJsonBody(request, 4 * 1024),
    );
    return noStoreJson(
      {
        reconciliation: await new PatientContextService(prisma).reconcile(
          actor,
          visitId,
    ...
    console.error("Patient Context reconciliation failed.", error);
    return noStoreJson(
      { error: { code: "INTERNAL_ERROR", message: "Patient Context reconciliation failed." } },
      { status: 500 },
    );
  }
}
