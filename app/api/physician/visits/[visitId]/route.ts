...
export async function GET(
  ...
  try {
    ...
    return noStoreJson(await new PhysicianVisitService(prisma).read(actor, visitId));
  } catch (error) {
    if (error instanceof AuthorizationError) return authorizationErrorResponse(error);
    if (error instanceof PhysicianVisitDraftError) {
      return physicianVisitDraftErrorResponse(error);
    }
    console.error("Physician Visit draft read failed.", error);
    return noStoreJson(
      { error: { code: "INTERNAL_ERROR", message: "Physician Visit draft read failed." } },
      { status: 500 },
    );
  }
}
